...
export async function runTurn(
  ...
  // Engine-owned intents (travel/rest/service) own their OWN mechanics — day,
  // location, gold, energy — so they SKIP Stage B entirely (the model can't drop a
  // field that breaks them) and go straight to narration. A null result falls back
  // to the normal model-driven Stage B (generic actions, or an unresolvable target).
  let effectsSummary: string;
  const engineSummary =
    actionIntent === "generic"
      ? null
      : resolveIntentAction(state, actionIntent, target, effects);

  if (engineSummary !== null) {
    recordScene(state, {}); // log the (possibly new) place; the player acts solo here
    updateMap(state); // derive world/local map from the engine-set location
    effectsSummary = `${engineSummary} ${summarizeEffects(state, effects)}`;
  } else {
    // --- Stage B: mechanical effects, choices, ending (model-driven) ---
    const agencyNote =
    ...
    effectsSummary = summarizeEffects(state, effects);
  }

  // --- Stage C: streamed prose narration (no tools, no JSON) ---
  // For engine-owned turns the narrator is told to honor the resolved outcome
  // exactly (don't invent an arrival mid-journey, show a refusal as a refusal).
  const narrationInstruction =
    engineSummary !== null
      ? `STAGE C. Narrate this turn in ENGLISH, in 2-3 vivid, second-person paragraphs, consistent with the RESOLVED OUTCOME shown above. The player's action: "${actionText}". Narrate EXACTLY what the engine resolved and do NOT contradict it: if the journey is unfinished, do not describe arriving; if a request was refused, show the refusal and why; reflect the gold, time, and stamina that actually changed. Then STOP at the player's next decision. Write ONLY the story prose — no choices, no lists, no JSON.`
